	    ...
	    <div class="row ${pageBean.totalSize<=0?"div1":"" }">
			...
			<nav aria-label="..." class="text-center">
			  <ul class="pagination">
			  	...
			  	<c:if test="${pageBean.pageNum>1 }">
			  	 	<li><a href="${pageContext.request.contextPath }/goodsservlet?method=getGoodsListByTypeId&pageNum=${pageBean.pageNum-1}&pageSize=${pageBean.pageSize}&typeId=${typeId}&goodsName=${goodsName}" aria-label="Previous"><span aria-hidden="true">«</span></a></li>
			  	</c:if>
			    ...
			    <c:forEach var="pn" begin="${pageBean.startPage }" end="${pageBean.endPage }" step="1">
			    	...
			    	<c:if test="${pn!=pageBean.pageNum }">
			    		<li ><a href="${pageContext.request.contextPath }/goodsservlet?method=getGoodsListByTypeId&pageNum=${pn }&pageSize=${pageBean.pageSize}&typeId=${typeId}&goodsName=${goodsName}">${pn }</a></li>
			    	</c:if>
			    </c:forEach>
			    ...
			    <c:if test="${pageBean.pageNum==pageBean.pageCount }">
			     <li class="disabled"><span aria-hidden="true">»</span></li>
			      ...
			      <c:if test="${pageBean.pageNum<pageBean.pageCount }">
			     <li><a href="${pageContext.request.contextPath }/goodsservlet?method=getGoodsListByTypeId&pageNum=${pageBean.pageNum+1}&pageSize=${pageBean.pageSize}&typeId=${typeId}&goodsName=${goodsName}" aria-label="Next"><span aria-hidden="true">»</span></a></li>
			    </c:if>
